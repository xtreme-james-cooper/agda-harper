...
finTric (FS x) (FS y) ngt neq with finTric x y (λ gt -> ngt (S>S gt)) (neqFSBackwards y x neq)
...
fincrSwapEq : {tn : nat} (t : fin tn) (x : fin (Suc tn)) -> fincr (fincr t x) (weaken x) == fincr (fincr t x) (FS x)
fincrSwapEq t      FZ     = Refl
fincrSwapEq FZ     (FS x) = Refl
fincrSwapEq (FS t) (FS x) rewrite fincrSwapEq t x = Refl
...
fincrSelf : {n : nat} (x : fin n) -> fincr x (FS x) == weaken x
fincrSelf FZ     = Refl
fincrSelf (FS x) rewrite fincrSelf x = Refl

fincrBelow : {n : nat} (x y : fin n) -> y >F x -> fincr y (FS x) == FS y
