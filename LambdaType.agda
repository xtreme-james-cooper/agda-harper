...
tsubstIncr t1 (TyVar tv)   x      y gt with finEq (fincr tv (FS x)) (weaken y)
tsubstIncr t1 (TyVar tv)   x      y gt | Yes eq with finEq tv y
...
tsubstIncr t1 (TyVar .x)   x      y gt | Yes eq | No neq  | No ngt  | Yes Refl rewrite fincrSelf x with neq (weakenEq eq)
...
tsubstIncr t1 (TyVar tv)   x      y gt | Yes eq | No neq  | No ngt  | No neq2  | lt rewrite fincrBelow x tv lt with tSubstIncrLemma (fgtTrans tv x y lt gt) eq
tsubstIncr t1 (TyVar tv)   x      y gt | Yes eq | No neq  | No ngt  | No neq2  | lt | ()
tsubstIncr t1 (TyVar tv)   x      y gt | No neq with finEq tv y
