...
data lam {n : nat} (gam : vect type n) : type -> Set where
  ...
  Abort : {t : type} -> lam gam Void -> lam gam t
  Inj : {t1 t2 : type} (d : direction) -> lam gam (proj d t1 t2) -> lam gam (t1 + t2)
  Case : {t1 t2 t : type} -> lam gam (t1 + t2) -> lam (t1 :: gam) t -> lam (t2 :: gam) t -> lam gam t
...
data isVal {n : nat} {gam : vect type n} : {t : type} -> lam gam t -> Set where
  ...
  InjVal : {t1 t2 : type} (d : direction) (e : lam gam (proj d t1 t2)) -> isVal e -> isVal (Inj d e)
...
data eval {n : nat} {gam : vect type n} : {t : type} -> lam gam t -> lam gam t -> Set where
  ...
  EvalRec1 : {t : type} {e e' : lam gam N} {e0 : lam gam t} {es : lam (t :: (N :: gam)) t} -> eval e e' -> eval (Rec e0 es e) (Rec e0 es e')
  EvalRec2 : {t : type} {e0 : lam gam t} {es : lam (t :: (N :: gam)) t} -> eval (Rec e0 es Zero) e0
  EvalRec3 : {t : type} {e : lam gam N} {e0 : lam gam t} {es : lam (t :: (N :: gam)) t} -> isVal e -> 
    eval (Rec e0 es (Succ e)) (subst FZ (subst FZ es (incr FZ (Rec e0 es e))) e)
  EvalPair1 : {t1 t2 : type} {e1 e1' : lam gam t1} {e2 : lam gam t2} -> eval e1 e1' -> eval (Pair e1 e2) (Pair e1' e2)
  EvalPair2 : {t1 t2 : type} {e1 : lam gam t1} {e2 e2' : lam gam t2} -> isVal e1 -> eval e2 e2' -> eval (Pair e1 e2) (Pair e1 e2')
  EvalProj1 : {t1 t2 : type} {d : direction} {e e' : lam gam (t1 X t2)} -> eval e e' -> eval (Proj d e) (Proj d e')
  EvalProj2 : {t1 t2 : type} {e1 : lam gam t1} {e2 : lam gam t2} -> isVal e1 -> isVal e2 -> eval (Proj L (Pair e1 e2)) e1
  EvalProj3 : {t1 t2 : type} {e1 : lam gam t1} {e2 : lam gam t2} -> isVal e1 -> isVal e2 -> eval (Proj R (Pair e1 e2)) e2
  EvalAbort : {t : type} {e e' : lam gam Void} -> eval e e' -> eval {t = t} (Abort e) (Abort e')
  EvalInj : {t1 t2 : type} {d : direction} {e e' : lam gam (proj d t1 t2)} -> eval e e' -> eval (Inj d e) (Inj d e')
  EvalCase1 : {t1 t2 t : type} {e e' : lam gam (t1 + t2)} {el : lam (t1 :: gam) t} {er : lam (t2 :: gam) t} -> eval e e' -> eval (Case e el er) (Case e' el er)
  EvalCase2 : {t1 t2 t : type} {e : lam gam t1} {el : lam (t1 :: gam) t} {er : lam (t2 :: gam) t} -> isVal e -> eval (Case (Inj L e) el er) (subst FZ el e)
  EvalCase3 : {t1 t2 t : type} {e : lam gam t2} {el : lam (t1 :: gam) t} {er : lam (t2 :: gam) t} -> isVal e -> eval (Case (Inj R e) el er) (subst FZ er e)
